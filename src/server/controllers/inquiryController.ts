...
import {
  generateScreenshotFingerprint,
  generateContentFingerprint,
} from "../utils/screenshotFingerprint.js";
...
export function getConversationTimeline(inquiry: any): any[] {
  if (
    inquiry.conversationHistory &&
    Array.isArray(inquiry.conversationHistory) &&
    inquiry.conversationHistory.length > 0
  ) {
  ...
  if (
    inquiry.clientAttachments &&
    Array.isArray(inquiry.clientAttachments) &&
    inquiry.clientAttachments.length > 0
  ) {
  ...
  if (
    inquiry.sentReply &&
    typeof inquiry.sentReply === "string" &&
    inquiry.sentReply.trim()
  ) {
    history.push({
 ...
 */
function isSemanticDuplicateString(
  candidate: string,
  existingList: string[],
): boolean {
  ...
  for (const item of existingList) {
    ...
    if (candTokens.length >= 2 && existTokens.length >= 2) {
      ...
      if (
        overlap.length / Math.min(candTokens.length, existTokens.length) >=
        0.7
      ) {
        return true;
      }
 ...
 */
function isItemInBaselineBreakdown(
  detName: string,
  ...
  for (const b of baselineBreakdown) {
    ...
    if (detTokens.length >= 2 && baseTokens.length >= 2) {
      ...
      if (
        overlap.length / Math.min(detTokens.length, baseTokens.length) >=
        0.65
      ) {
        return true;
      }
...
export async function createInquiry(
  ...
  try {
    ...
    const finalSourceChannel =
      sourceChannel || (sourceType === "screenshot" ? "Fiverr" : "Fiverr");
    const inputName = (clientName || "").trim();
    const inputEmail = (clientEmail || "").trim().toLowerCase();
    const targetSubject = (
      subject ||
      (sourceType === "screenshot"
        ? "Fiverr Screenshot Inquiry"
    ...
    if (isUsingMemoryDB()) {
      ...
      if (clientId && clientId !== "new") {
        resolvedClient = memoryStore.clients.find(
          (c) =>
            (c.id === clientId || c._id === clientId) && c.userId === userId,
        );
      }
      ...
      if (!resolvedClient) {
        ...
        if (!resolvedClient && inputName) {
          resolvedClient = memoryStore.clients.find(
            (c) =>
              c.userId === userId &&
              c.name?.toLowerCase() === inputName.toLowerCase(),
          );
        }
      }
      ...
      if (resolvedClient) {
        ...
        const newCliId =
          "cli_" + Date.now() + Math.random().toString(36).substr(2, 4);
        resolvedClient = {
          id: newCliId,
          _id: newCliId,
          userId,
      ...
      const resolvedClientId = resolvedClient
        ? resolvedClient.id || resolvedClient._id
        : undefined;
      const resolvedClientName = resolvedClient
        ? resolvedClient.name
        : inputName || "Client";
      const resolvedClientEmail = resolvedClient
        ? resolvedClient.email || inputEmail
        : inputEmail;
      ...
      const existingInquiry = memoryStore.inquiries.find((inq) => {
        ...
        const matchClient = resolvedClientId
          ? inq.clientId === resolvedClientId ||
            (inq.clientEmail &&
              inq.clientEmail.toLowerCase() === resolvedClientEmail) ||
            inq.clientName?.toLowerCase() === resolvedClientName.toLowerCase()
          : inq.clientName?.toLowerCase() === resolvedClientName.toLowerCase();

        const matchSubject =
          (inq.subject || "").trim().toLowerCase() ===
          targetSubject.toLowerCase();
      ...
      const inqId =
        "inq_" + Date.now() + Math.random().toString(36).substr(2, 4);
      const initialAttachments: any[] = [];
      const initialHistory: any[] = [];

      if (screenshotData) {
        const attId =
          "att_" + Date.now() + Math.random().toString(36).substring(2, 6);
        ...
        initialHistory.push({
          ...
          text:
            extractedMessageText ||
            (finalRawMessage.startsWith("[Fiverr Screenshot Uploaded")
              ? ""
              : finalRawMessage),
          read: false,
          createdAt: new Date().toISOString(),
...
export async function updateInquiry(
  ...
  try {
    ...
    if (isUsingMemoryDB()) {
      ...
      if (
        updates.sentReply &&
        typeof updates.sentReply === "string" &&
        updates.sentReply.trim()
      ) {
        ...
        if (!updates.conversationHistory) {
          const lastMsg =
            inq.conversationHistory[inq.conversationHistory.length - 1];
          if (
            !lastMsg ||
            lastMsg.sender !== "freelancer" ||
            lastMsg.text !== updates.sentReply
          ) {
      ...
      if (updates.starred !== undefined) {
        ...
        inq.starredAt = inq.starred
          ? updates.starredAt || new Date().toISOString()
          : undefined;
      }
    ...
    if (
      ...
      typeof updates.sentReply === "string" &&
      updates.sentReply.trim() &&
      (!updates.conversationHistory || updates.conversationHistory.length === 0)
    ) {
      const existingDoc = await Inquiry.findOne({ _id: id, userId });
      if (existingDoc) {
        const history =
          existingDoc.conversationHistory &&
          existingDoc.conversationHistory.length > 0
            ? existingDoc.conversationHistory
...
export async function toggleStarInquiry(
  ...
  try {
    ...
    if (isUsingMemoryDB()) {
      ...
      const nextStarred =
        typeof starred === "boolean" ? starred : !Boolean(inq.starred);
    ...
    const nextStarred =
      typeof starred === "boolean" ? starred : !Boolean(existingDoc.starred);
...
export async function analyzeInquiry(
  ...
  try {
    ...
    const hasIncomingScreenshot = Boolean(
      req.body &&
      req.body.screenshotData &&
      req.body.screenshotData.trim().length > 0,
    );
    const targetScreenshotData = hasIncomingScreenshot
      ? req.body.screenshotData
      : inquiry.screenshotData;
    ...
    const targetFileName = hasIncomingScreenshot
      ? req.body.fileName ||
        `Fiverr_Attachment_${(inquiry.clientAttachments?.length || 0) + 1}.png`
        ...
        inquiry.analyzedScreenshots.push({
          ...
          analysisResult: {
            ...
            extractedSkillsRequired:
              inquiry.analysisResult.requiredSkills || [],
            scopeComplexity: inquiry.analysisResult.scopeComplexity || "medium",
            detectedItems:
              inquiry.analysisResult.pricingEstimate?.breakdown?.map(
                (b: any) => ({
                  name:
                    typeof b.item === "string"
                      ? b.item.replace(/\s*\(x\d+\)$/, "")
            ...
            informationToClarify:
              inquiry.analysisResult.questionsToClarify || [],
            extractedMessageText:
              inquiry.analysisResult.extractedMessageText ||
              inquiry.extractedMessageText ||
              "",
          },
          extractedMessageText: inquiry.extractedMessageText || "",
          analyzedAt:
            inquiry.analysisResult.analyzedAt ||
            inquiry.createdAt ||
            new Date().toISOString(),
        });
      }
    ...
    const alreadyAnalyzed = Boolean(
      targetFingerprint &&
      (inquiry.analyzedScreenshots || []).some(
        (item: any) => item.fingerprint === targetFingerprint,
      ),
    );
    ...
    if (alreadyAnalyzed && inquiry.analysisResult) {
      ...
      if (hasIncomingScreenshot) {
        const isAlreadyInAttachments = (inquiry.clientAttachments || []).some(
          (att: any) => {
            return (
              generateScreenshotFingerprint(att.data) === targetFingerprint
            );
          },
        );
    ...
    if (isUsingMemoryDB()) {
      ...
      if (
        !memUser ||
        (memUser.aiCreditsRemaining ?? PRO_PLAN_AI_CREDITS_LIMIT) < 1
      ) {
        res.status(402).json({
          success: false,
    ...
    if (extraction.extractedMessageText) {
      const lastClientMsg = [...inquiry.conversationHistory]
        .reverse()
        .find((m: any) => m.sender === "client");
      if (
        lastClientMsg &&
        (!lastClientMsg.text ||
          lastClientMsg.text.startsWith("[Fiverr Screenshot"))
      ) {
...
export async function translateInquiryAnalysis(
  ...
  try {
    ...
    if (isUsingMemoryDB()) {
      inquiry = memoryStore.inquiries.find(
        (i) =>
          ...
          (String(i.userId) === String(userId) || !i.userId),
      );
    } else {
    ...
    if (
      ...
      inquiry.analysisResult.translations[targetLanguage]
    ) {
      console.log(
    ...
    if (
      ...
      res.status(429).json({
        ...
        error: "GEMINI_QUOTA_EXHAUSTED",
        message:
          "AI translation service quota has been temporarily reached. The English analysis remains available.",
      });
      return;
    }
    ...
    if (
      ...
      res.status(503).json({
        ...
        error: "GEMINI_MODEL_UNAVAILABLE",
        message:
          "AI translation model service is temporarily unavailable. Please try again in a moment.",
      });
      return;
    }
    ...
    res.status(500).json({
      ...
      message:
        error?.message || "Failed to translate inquiry analysis into Bengali.",
    });
  }
}
