...
async function connectDB() {
  ...
  try {
    ...
    await mongoose2.connect(ENV.MONGODB_URI, {
...
init_User();
init_types();
var ServiceItemSchema = new Schema2(
  {
    id: { type: String },
...
var UserProfileSchema = new Schema2(
  {
    userId: {
...
var UserProfile = mongoose3.models.UserProfile || mongoose3.model("UserProfile", UserProfileSchema);
var SALT_ROUNDS = 10;
...
var baseInquirySchema = z.object({
...
var Inquiry = mongoose4.models.Inquiry || mongoose4.model("Inquiry", InquirySchema);
var ClientSchema = new Schema4(
  {
    userId: {
...
var Client = mongoose5.models.Client || mongoose5.model("Client", ClientSchema);
init_User();
...
var Notification = mongoose6.models.Notification || mongoose6.model("Notification", NotificationSchema);
...
var ExtractionZodSchema = z2.object({
  clientWants: z2.array(z2.string()),
  extractedSkillsRequired: z2.array(z2.string()),
  scopeComplexity: z2.enum(["low", "medium", "high"]).catch("medium"),
  detectedItems: z2.array(
    z2.object({
      name: z2.string(),
      qty: z2.number().catch(1)
    })
  ),
  informationToClarify: z2.array(z2.string()),
  extractedMessageText: z2.string().catch("")
});
function delay(ms) {
...
async function translateInquiryAnalysis(req, res, next) {
  ...
  try {
    ...
    if (isUsingMemoryDB()) {
      ...
      const isObjId = mongoose7.Types.ObjectId.isValid(id);
      const isUserObjId = mongoose7.Types.ObjectId.isValid(userId);
    ...
    if (isUsingMemoryDB()) {
      ...
      const isDocObjId = mongoose7.Types.ObjectId.isValid(inquiry._id || id);
      if (isDocObjId) {
        const updatedDoc = await Inquiry.findOneAndUpdate(
          { _id: inquiry._id || id },
          {
...
init_User();
var TemplateSchema = new Schema6(
  {
    userId: {
...
var Template = mongoose8.models.Template || mongoose8.model("Template", TemplateSchema);
init_planConfig();
var analyticsQuerySchema = z3.object({
  period: z3.enum(["7d", "30d", "90d", "year", "all", "custom"]).optional().default("30d"),
...
async function getAnalytics(req, res, next) {
  try {
    ...
    const userObjectId = new mongoose9.Types.ObjectId(userId);
...
var FollowUpSchema = new Schema7(
  {
    userId: {
...
var FollowUp = mongoose10.models.FollowUp || mongoose10.model("FollowUp", FollowUpSchema);
init_User();
...
function createApp() {
  ...
  app2.use((req, _res, next) => {
    const forwarded = req.headers["x-forwarded-uri"] || req.headers["x-matched-path"] || req.headers["x-vercel-matched-path"] || "";
  ...
  app2.use(simpleCookieParser);
  app2.use(noSqlSanitizerMiddleware);
  ...
  app2.get("/api/health", handleHealth);
  app2.get("/api/v1/health", handleHealth);
  ...
  app2.use("/api", apiGeneralRateLimiter);
  app2.use("/api/auth", authRoutes_default);
  app2.use("/api/profile", profileRoutes_default);
  app2.use("/api/templates", templateRoutes_default);
  app2.use("/api/notifications", notificationRoutes_default);
  app2.use("/api/followups", followUpRoutes_default);
  app2.use("/api/inquiries", inquiryRoutes_default);
  app2.use("/api/analytics", analyticsRoutes_default);
  app2.use("/api/usage", usageRoutes_default);
  app2.use((req, res) => {
    res.status(404).json({
      success: false,
